...
import com.ssafy.businesscard.user.controller.WebClientConfig;
...
public class OAuthService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        log.info("sssssssssssssssssssssssssss" + userRequest.getAccessToken().getTokenValue());
        String accesstoken = userRequest.getAccessToken().getTokenValue();

        String result = webClientConfig.webClient().get()
                .uri("https://graph.microsoft.com/v1.0/me") // 호출할 엔드포인트
                .header("Authorization", "Bearer" + accesstoken)
                .retrieve() // 응답 검색

                .bodyToMono(String.class) // 응답 본문을 String으로 변환
                .block(); // 비동기 호출을 동기 호출로 변환 (주의: 블로킹 연산)

        // 결과 처리 로직...
        log.info("resssssss"+result);
        ...
        if (attributes == null) {
            // 속성값 사용
            log.info(".getAttribute(String)\" is null");
        }

        log.info("after" + attributes);

        MemberProfile memberProfile = OAuthAttributes.extract(registrationId, attributes); // registrationId에 따라 유저 정보를 통해 공통된   객체로 만들어 줌
        ...
        log.info("customAttributecustomAttribute" + customAttribute);
    ...
    private Map customAttribute(Map attributes, String userNameAttributeName, MemberProfile memberProfile, String registrationId) {
        ...
        log.info("userNameAttributeName" + attributes.get(userNameAttributeName));
