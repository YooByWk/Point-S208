...
const TeamMember = ({team}: {team: TeamListType}) => {
  ...
  const filteredMembers = Array.isArray(teamMember) && !isLoading ? teamMember.filter((member) =>
  ...
  return (
    ...
    {isLoading ? (
      ...
      <>
        ...
        <div css={btn}>
        <LargeButton text='팀원 추가' width='80%' onClick={()=>{setAddingMember(true)}} />
        </div>
      </>):(<AddMember team={team} state={{value: addingMember ,setValue:setAddingMember}}/>)
    )}
  </div>
  );
};
